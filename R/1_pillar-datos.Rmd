...
Los datos están en la pagina: http://www.cuartopoder.es/multimedia/2014/10/11/gastos-de-los-exdirectivos-de-caja-madrid-uno-a-uno-con-las-tarjetas-negras-tabla/3403.
...
Descargamos e importamos el fichero *.csv* que esta en [github](https://github.com/fdelaunay/tutorial-dplyr-es/tree/master/data). Esta lista proviene de [Elpais](http://elpais.com/especiales/2014/tarjetas-opacas-caja-madrid/).
 ...
 * transformamos los importes decimales (limitados a 2 cifras):
    ...
    ```{r}
    movimientos[,importe := round(as.numeric(importe, 2))]
    ```
...
```{r}
library(dplyr)
tar_mov <- join(movimientos, tarjetas, by="tarjeta_id")
head(tar_mov)
```
...
data <- anti_join(personas, tar_mov, by="persona")

data[order(data$persona),"persona"]
```

¡problema! el **anti_join** devuelve 67 personas. Siñifica que 67 de las 80 filas de la tabla persona que no tienen equivalente en la tabla mov_tar. Esta debido a los accentos, `personas$persona` tiene acentos, `tar_mov$persona` no.
